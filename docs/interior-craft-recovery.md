...
- 최종 이미지 규격: **3840×2160 고해상도 JPEG**. 640×360 원본 프레임은 Lanczos 확대·선명화 후 교체하며, 검증 완료 뒤 저해상도 원본은 남기지 않는다.
...
- 반복 출제 유형과 단독 출제 참고를 분리해 표시하며, 연도별 상한은 적용하지 않는다.
- 빈도점수 = 출제연도수×3 + 실제 출제횟수. 최신점수는 2024·2025 출제연도 수로 별도 표시만 하고 순위에 곱하지 않는다.
- 순위는 반복 유형 우선 → 빈도점수 → 출제연도수 → 실제 출제횟수 → 안정 ID 순으로 결정한다.
- 이미지 재판독으로 핵심 개념·질문 의도·풀이법을 교차 확인하고, 다른 유형은 분리
- 파일명·노트 번호·화면 좌상단 실제 번호가 다르면 해당 문항을 반복유형 링크에서 제외
- 전면검수 후 자격 전체 상위 100을 다시 계산. 2026 성안당 주제 자료는 실제 빈도 입력에서 제외
