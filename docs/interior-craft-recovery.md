...
- 대표는 텍스트 우선, 없으면 이미지
- 단독 출제 참고 69개는 기존 선정 후보를 유지하고 단독우선점수로 재정렬한다. 관련개념 출제연도수×3 + 관련개념 실제 출제횟수 + 핵심성×2 + 수치·규정성 + 기출 활용성으로 계산한다.
- 단독 후보의 관련개념은 구체 개념 키로만 집계하며 `기타`나 광범위한 공통어는 관련 빈도 0으로 처리한다. 최신점수는 표시만 한다.
